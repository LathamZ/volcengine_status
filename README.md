...
- **菜单栏标题**展示 Agent / Coding 套餐的*剩余*百分比(如 `A 0%·W | C 30%·S`):先百分比,`·` 后跟窗口标识(`5h`/`S`/`W`/`M`),` | ` 分隔套餐。可选「自动」档:每个套餐自动盯最接近耗尽的那个窗口(任一窗口满即停服)。展示哪些套餐、取哪个周期(自动 / 短周期 / weekly / monthly)均可在设置里切换。
- **浮层**展示各周期进度条:
